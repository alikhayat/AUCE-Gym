...
Public Class cochadd
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        Else
            ...
            Dim coachid As Integer = Convert.ToInt32(main.DataGridView11.Rows(0).Cells(3).Value.ToString) + 1
            Dim queryyy As String = "UPDATE [varr] SET [cochid]=? WHERE [ID]=? "
            Using conn = New OleDb.OleDbConnection("PROVIDER=Microsoft.ACE.OLEDB.12.0;DATA SOURCE=" & main.path + "varr.mdb;Jet OLEDB:Database Password=janitani2;")
                ...
                Using cmd = New OleDbCommand(queryyy, conn)
                    conn.Open()
                    cmd.Parameters.AddWithValue("@p1", Convert.ToString(coachid))
            ...
            End Using
            con.Open()
            Dim cochid As String = Convert.ToString(coachid)
            Dim strSql = ("insert into coaches values ('" + cochid + "', '" + TextBox1.Text + "', '" + TextBox2.Text + "', '" + TextBox3.Text + "', '" + DateTimePicker1.Value.ToShortDateString + "', '" + TextBox4.Text + "', '" + ComboBox1.Text + "', '" + TextBox5.Text + "', '" + TextBox6.Text + "', '" + _
