    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        Else
            Try
                ...
                Dim con As New OleDb.OleDbConnection("PROVIDER=Microsoft.ACE.OLEDB.12.0;DATA SOURCE=" & main.path + "classes.mdb;Jet OLEDB:Database Password=janitani2;")
                'My.Settings.sesid += 1
                Dim sesid As Integer = Convert.ToInt32(main.DataGridView11.Rows(0).Cells(2).Value.ToString) + 1

                Dim queryyy As String = "UPDATE [varr] SET [sesid]=? WHERE [ID]=? "
                Using conn = New OleDb.OleDbConnection("PROVIDER=Microsoft.ACE.OLEDB.12.0;DATA SOURCE=" & main.path + "varr.mdb;Jet OLEDB:Database Password=janitani2;")
                    ...
                    Using cmd = New OleDbCommand(queryyy, conn)
                        conn.Open()
                        cmd.Parameters.AddWithValue("@p1", sesid.ToString)
                ...
                End Using
                con.Open()
                Dim strSql = ("insert into classes values ('" + sesid.ToString + "', '" + TextBox1.Text + "', '" + ComboBox1.Text + "', '" + checkeditems + "', '" + DateTimePicker1.Value.ToShortTimeString + "', '" + DateTimePicker2.Value.ToShortTimeString + "', '" + Label12.Text + "', '" + ComboBox4.Text + "', '" + ComboBox2.Text + "', '" + ComboBox6.Text + "', '" + TextBox2.Text + "', '" + "No" + "', '" + "to be detemined" + "')")
